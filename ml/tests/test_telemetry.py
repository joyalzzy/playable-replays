...
class TelemetryTests(unittest.TestCase):
    def test_specific_semantic_categories_outrank_generic_proximity(self):
        cases = (
            (("team-fight", "team-fight-reversal"), "team-fight-engagement"),
            (("team-fight", "successful-escape"), "escape"),
            (("team-fight", "objective-contest"), "objective-contest"),
            (("team-fight", "vision-uncertainty"), "vision-uncertainty"),
            (("team-fight", "resource-disadvantage"), "resource-trade"),
            (("team-fight", "one-versus-many"), "positioning"),
            (("team-fight",), "team-fight-engagement"),
        )
        for reason_tags, expected in cases:
            with self.subTest(reason_tags=reason_tags):
                self.assertEqual(category_for_reason_tags(reason_tags), expected)
    ...
    def test_accepts_normalized_vision_loss_event(self):
        checked = validate_frames(
            (
                frame(0, 0.5, events=("vision-loss",)),
                frame(1, 0.5),
            )
        )

        self.assertEqual(checked[0].event_types, ("vision-loss",))
