...
func (m *HTTPModel) NextPositions(ctx context.Context, snapshot engine.OpponentSnapshot) (engine.OpponentModelResult, error) {
	...
	if err != nil {
		return engine.OpponentModelResult{}, fmt.Errorf("encode position snapshot: %w", err)
	}
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, m.endpoint.String(), bytes.NewReader(body))
	if err != nil {
		return engine.OpponentModelResult{}, fmt.Errorf("create position model request: %w", err)
	}
	...
	if err != nil {
		return engine.OpponentModelResult{}, errors.New("position model request failed")
	}
	defer res.Body.Close()
	if res.StatusCode < http.StatusOK || res.StatusCode >= http.StatusMultipleChoices {
		_, _ = io.Copy(io.Discard, io.LimitReader(res.Body, maxResponseBytes))
		return engine.OpponentModelResult{}, fmt.Errorf("position model returned status %d", res.StatusCode)
	}

	limited := io.LimitReader(res.Body, maxResponseBytes+1)
	data, err := io.ReadAll(limited)
	if err != nil {
		return engine.OpponentModelResult{}, errors.New("read position model response")
	}
	if len(data) > maxResponseBytes {
		return engine.OpponentModelResult{}, errors.New("position model response is too large")
	}
	var decoded response
	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	if err := decoder.Decode(&decoded); err != nil {
		return engine.OpponentModelResult{}, errors.New("position model returned malformed JSON")
	}
	if decoder.Decode(&struct{}{}) != io.EOF {
		return engine.OpponentModelResult{}, errors.New("position model response must contain one JSON value")
	}
	if decoded.Positions == nil {
		return engine.OpponentModelResult{}, errors.New("position model response must contain a positions array")
	}
	if len(*decoded.Positions) > engine.MaxSnapshotUnits {
		return engine.OpponentModelResult{}, errors.New("position model returned too many positions")
	}
	...
	for _, wire := range *decoded.Positions {
		if wire.UnitID == nil || *wire.UnitID == "" {
			return engine.OpponentModelResult{}, errors.New("position model returned an empty unit ID")
		}
		if wire.Position == nil || wire.Position.X == nil || wire.Position.Y == nil {
			return engine.OpponentModelResult{}, errors.New("position model returned an incomplete position")
		}
		...
		if _, duplicate := seen[suggestion.UnitID]; duplicate {
			return engine.OpponentModelResult{}, errors.New("position model returned a duplicate unit ID")
		}
		seen[suggestion.UnitID] = struct{}{}
		if !finite(suggestion.Position.X) || !finite(suggestion.Position.Y) {
			return engine.OpponentModelResult{}, errors.New("position model returned a non-finite position")
		}
		if suggestion.Position.X < snapshot.MapBounds.MinX || suggestion.Position.X > snapshot.MapBounds.MaxX ||
			...
			return engine.OpponentModelResult{}, errors.New("position model returned an out-of-bounds position")
		}
