...
func NewHTTPModel(rawURL string, client *http.Client) (*HTTPModel, error) {
	...
	return &HTTPModel{endpoint: endpoint, client: client}, nil
}

func (m *HTTPModel) NextPositions(ctx context.Context, snapshot engine.OpponentSnapshot) ([]engine.PositionSuggestion, error) {
	body, err := json.Marshal(snapshot)
	if err != nil {
		return nil, fmt.Errorf("encode opponent snapshot: %w", err)
	}
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, m.endpoint.String(), bytes.NewReader(body))
	if err != nil {
		return nil, fmt.Errorf("create opponent request: %w", err)
	}
	...
	if err != nil {
		return nil, errors.New("opponent model request failed")
	}
	defer res.Body.Close()
	if res.StatusCode < http.StatusOK || res.StatusCode >= http.StatusMultipleChoices {
		_, _ = io.Copy(io.Discard, io.LimitReader(res.Body, maxResponseBytes))
		return nil, fmt.Errorf("opponent model returned status %d", res.StatusCode)
	}

	limited := io.LimitReader(res.Body, maxResponseBytes+1)
	data, err := io.ReadAll(limited)
	if err != nil {
		return nil, errors.New("read opponent model response")
	}
	if len(data) > maxResponseBytes {
		return nil, errors.New("opponent model response is too large")
	}
	var decoded response
	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	if err := decoder.Decode(&decoded); err != nil {
		return nil, errors.New("opponent model returned malformed JSON")
	}
	if decoder.Decode(&struct{}{}) != io.EOF {
		return nil, errors.New("opponent model response must contain one JSON value")
	}
	if decoded.Positions == nil {
		return nil, errors.New("opponent model response must contain a positions array")
	}
	if len(*decoded.Positions) > engine.MaxSnapshotUnits {
		return nil, errors.New("opponent model returned too many positions")
	}
	...
	for _, wire := range *decoded.Positions {
		if wire.UnitID == nil || *wire.UnitID == "" {
			return nil, errors.New("opponent model returned an empty unit ID")
		}
		if wire.Position == nil || wire.Position.X == nil || wire.Position.Y == nil {
			return nil, errors.New("opponent model returned an incomplete position")
		}
		...
		if _, duplicate := seen[suggestion.UnitID]; duplicate {
			return nil, errors.New("opponent model returned a duplicate unit ID")
		}
		seen[suggestion.UnitID] = struct{}{}
		if !finite(suggestion.Position.X) || !finite(suggestion.Position.Y) {
			return nil, errors.New("opponent model returned a non-finite position")
		}
		if suggestion.Position.X < snapshot.MapBounds.MinX || suggestion.Position.X > snapshot.MapBounds.MaxX ||
			...
			return nil, errors.New("opponent model returned an out-of-bounds position")
		}
